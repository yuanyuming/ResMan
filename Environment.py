...
class Allocation_Environment:
    def __init__(self, pa=parameters.Parameters(), nw_len_seq=None, nw_size_seq=None,
        ...
        if nw_len_seq is None or nw_size_seq is None:
            ...
            self.nw_len_seq, self.nw_size_seq = \
                self.generate_sequence_work(
                    self.pa.simulate_len*self.pa.num_ex)
            ...
            for i in range(pa.num_res):
                ...
                print("Load On #", i, "Resource Dimension Is ",
                      self.workload[i])
        ...
        self.machine = Machine.Machine(
            pa.num_res, pa.time_horizon, pa.res_slot, pa.job_num_cap)
    ...
    def compact_repre(self):
        ...
        job_allcoated = np.ones(self.pa.time_horizon) * \
        ...
        # 当前工作允许的时间间隙
        for i in range(self.pa.num_res):
            compact_repre[cr_pt:cr_pt +
                          self.pa.time_horizon] = self.machine.avail_slot[:, i]
